...
def show_banner(user_id, user_name):
 ...
 |______|______\____/|_| \_|_|  |_|\____/|_____/|_|\_\ """)
    ...
    print(f"{C_WHITE}  🔥 SEASON  : {C_MAGENTA} SEASON 2 VIP USER ")
...
class RuijieLoginManager:
    ...
    async def _fetch_sid(self, session):
        current_ip = self.ip if self.ip else "192.168.110.158"
        current_gw = self.ip if self.ip else "192.168.110.1"
        current_mac = self.mac if self.mac else "da:3b:e2:43:ca:e7"
        ...
        params = {
            ...
            "chap_id": r"\243",
            "chap_challenge": r"\330\323\377\244\174\166\021\234\301\124\370\171\015\155\360\016"
        }
